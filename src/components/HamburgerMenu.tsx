...
export function HamburgerMenu({ onToggleHistory, onOpenSettings, sidebarOpen }: HamburgerMenuProps) {
  ...
  const handleItemClick = useCallback((item: 'updates' | 'help' | 'history' | 'settings') => {
    if (item === 'history') {
      onToggleHistory();
      handleClose();
  ...
  }, [activeSub, onToggleHistory, onOpenSettings, handleClose]);
